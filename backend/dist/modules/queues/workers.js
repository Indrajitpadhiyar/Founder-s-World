...
export const initWorkers = () => {
    ...
    const simulationWorker = new Worker('simulationQueue', async (job) => {
        try {
            ...
            if (job.name === 'factoryProductionTick') {
                ...
                completedProductions.forEach((prod) => {
                    SocketService.sendToUser(prod.userId.toString(), 'factoryProductionCompleted', {
            ...
            logger.error(`Error in simulation worker job ${job.name}: ${err.message}`);
            throw err;
        }
