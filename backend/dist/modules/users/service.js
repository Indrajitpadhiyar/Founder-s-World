...
export class UserService {
    ...
    async getLeaderboard(limit = 10) {
        ...
        if (redisClient) {
            try {
                const topUsers = await redisClient.zrevrange(LEADERBOARD_KEY, 0, limit - 1, 'WITHSCORES');
    ...
    async addWealth(userId, amount) {
        const user = await User.findByIdAndUpdate(userId, { $inc: { cash: amount, netWorth: amount } }, { new: true });
        if (user && redisClient) {
