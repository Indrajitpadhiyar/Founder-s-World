...
export class FactoryService {
    async createFactory(userId, name, productId) {
        ...
        try {
            ...
            if (user.cash < startupCost) {
                throw new BadRequestError(`Insufficient cash. Constructing a factory costs $${startupCost}.`);
            }
    ...
    async upgradeFactory(userId, factoryId) {
        ...
        try {
            ...
            const upgradeCost = Math.round(50000 * Math.pow(1.8, factory.upgradeLevel));
            const user = await User.findById(userId).session(session);
            if (!user) {
                throw new NotFoundError('User not found');
            }
            if (user.cash < upgradeCost) {
                throw new BadRequestError(`Insufficient cash. Upgrading costs $${upgradeCost}.`);
            }
        ...
        for (const factory of activeFactories) {
            ...
            if (factory.productionProgress >= 100) {
                ...
                try {
                    ...
                    const warehouse = await Warehouse.findOne({ userId: factory.userId }).session(session);
                    if (warehouse) {
                        const product = await Product.findById(factory.productId).session(session);
                        if (product && warehouse.usedCapacity < warehouse.capacity) {
                            ...
                            if (invItem) {
                                const totalCost = (invItem.averageCost * invItem.quantity) + costOfProduction;
                                invItem.quantity += 1;
