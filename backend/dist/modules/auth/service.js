...
export class AuthService {
    ...
    async register(userData) {
        ...
        const existingUsername = await this.authRepository.findUserByUsername(userData.username);
        if (existingUsername) {
        ...
        const newUser = await this.authRepository.createUser({
            ...
            xp: 0,
            level: 1,
            cash: 100000, // $100,000 starting money
            netWorth: 100000,
        });
        return newUser;
    }
    async login(credentials, device, ip) {
        ...
        await this.authRepository.createRefreshToken(user.id, refreshToken, expiresAt, device, ip);
    ...
    async refresh(token, device, ip) {
        ...
        await this.authRepository.createRefreshToken(payload.userId, newRefreshToken, expiresAt, device, ip);
