...
export class BusinessService {
    ...
    try {
      ...
      if (user.cash < config.startupCost) {
        throw new BadRequestError(
          `Insufficient funds. You need $${config.startupCost} to start a ${category} business.`,
        );
      }
  ...
  async upgradeBusiness(userId, businessId) {
    ...
    try {
      ...
      const upgradeCost = Math.round(
        config.startupCost * Math.pow(1.5, business.level),
      );
      ...
      if (user.cash < upgradeCost) {
        throw new BadRequestError(
          `Insufficient funds. Upgrading to level ${business.level + 1} costs $${upgradeCost}.`,
        );
      }
      ...
      business.level += 1;
      business.revenueRate = Math.round(
        config.baseRevenue * Math.pow(1.3, business.level - 1),
      );
      business.expenseRate = Math.round(
        config.baseExpense * Math.pow(1.2, business.level - 1),
      );
      business.marketShare = Math.min(
        30,
        business.marketShare + 0.15 * business.level,
      ); // Max 30% share
  ...
  async collectRevenue(userId, businessId) {
    ...
    try {
      ...
      const now = new Date();
      const diffMs =
        now.getTime() - new Date(business.lastCollectedAt).getTime();
      const diffMinutes = diffMs / (1000 * 60);

      if (diffMinutes < 0.05) {
        throw new BadRequestError(
          "Must wait at least 3 seconds between cash collections",
        );
      }
