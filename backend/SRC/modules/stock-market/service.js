...
export class StockMarketService {
  ...
  async buyStockMarket(userId, stockCompanyId, sharesCount) {
    ...
    try {
      const stock =
        await StockCompany.findById(stockCompanyId).session(session);
      if (!stock) {
      ...
      if (user.cash < totalCost) {
        throw new BadRequestError(
          `Insufficient cash. You need $${totalCost} but only have $${user.cash}.`,
        );
      }
      ...
      if (portfolio) {
        ...
        const totalSpent =
          portfolio.averageCost * portfolio.sharesOwned + totalCost;
      ...
      stock.currentPrice = parseFloat(
        (stock.currentPrice * (1 + 0.0005 * sharesCount)).toFixed(2),
      );
      ...
      await StockPrice.create(
        [{ stockCompanyId, price: stock.currentPrice, timestamp: new Date() }],
        { session },
      );
  ...
  async sellStockMarket(userId, stockCompanyId, sharesCount) {
    ...
    try {
      ...
      if (!portfolio || portfolio.sharesOwned < sharesCount) {
        throw new BadRequestError(
          `You do not own enough shares to sell. Owned: ${portfolio?.sharesOwned || 0}`,
        );
      }
      ...
      stock.currentPrice = parseFloat(
        Math.max(0.01, stock.currentPrice * (1 - 0.0005 * sharesCount)).toFixed(
          2,
        ),
      );
      stock.marketCap = Math.round(stock.currentPrice * stock.sharesIssued);
      ...
      return {
        portfolio: portfolio.sharesOwned > 0 ? portfolio : null,
        cash: user.cash,
      };
    } catch (error) {
      await session.abortTransaction();
  ...
  async ipoBusiness(userId, businessId, ticker) {
    ...
    try {
      ...
      const existingStock = await StockCompany.findOne({
      ...
      const existingTicker = await StockCompany.findOne({
      ...
      if (valuation < 100000000) {
        throw new BadRequestError(
          `Business valuation is too low: $${valuation.toLocaleString()}. Min required is $100,000,000. Upgrade business to increase value.`,
        );
      }
  ...
  async tickStockSimulation() {
    ...
    for (const stock of stocks) {
      ...
      stock.currentPrice = parseFloat(
        Math.max(0.01, oldPrice * multiplier).toFixed(2),
      );
      stock.marketCap = Math.round(stock.currentPrice * stock.sharesIssued);
      await stock.save();
      ...
      if (redisClient) {
        await redisClient
          .set(`stock:ticker:${stock.ticker}`, stock.currentPrice)
          .catch(() => {});
      }
