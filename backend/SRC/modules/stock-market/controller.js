...
export class StockMarketController {
  ...
  getStocks = async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .json(successResponse("Stock list fetched successfully", stocks));
    } catch (error) {
      next(error);
    }
  };
  ...
  getHistory = async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .json(successResponse("Stock history fetched successfully", history));
    } catch (error) {
      next(error);
    }
  };
  ...
  getPortfolio = async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .json(
          successResponse("User portfolio fetched successfully", portfolio),
        );
    } catch (error) {
      next(error);
    }
  ...
  buyStock = async (req, res, next) => {
    try {
      ...
      const result = await this.stockService.buyStockMarket(
        userId,
        stockCompanyId,
        sharesCount,
      );
      res
        .status(200)
        .json(successResponse("Stock purchased successfully", result));
    } catch (error) {
      next(error);
    }
  };
  ...
  sellStock = async (req, res, next) => {
    try {
      const userId = req.user.id;
      const { stockCompanyId, sharesCount } = req.body;
      const result = await this.stockService.sellStockMarket(
        userId,
        stockCompanyId,
        sharesCount,
      );
  ...
  launchIpo = async (req, res, next) => {
    try {
      ...
      const result = await this.stockService.ipoBusiness(
        userId,
        businessId,
        ticker,
      );
      res
        ...
        .json(
          successResponse(
            "IPO completed successfully! Stock is listed.",
            result,
          ),
        );
    } catch (error) {
