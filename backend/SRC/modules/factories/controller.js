...
export class FactoryController {
  ...
  create = async (req, res, next) => {
    try {
      ...
      res
        .status(201)
        .json(successResponse("Factory constructed successfully", factory));
    } catch (error) {
      next(error);
    }
  };
  ...
  list = async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .json(successResponse("Factories fetched successfully", factories));
    } catch (error) {
      next(error);
    }
  };
  ...
  upgrade = async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .json(successResponse("Factory upgraded successfully", factory));
    } catch (error) {
      next(error);
    }
  };
  ...
  toggle = async (req, res, next) => {
    try {
      ...
      res
        ...
        .json(
          successResponse(
            `Production ${factory.isProducing ? "started" : "stopped"} successfully`,
            factory,
          ),
        );
    } catch (error) {
